...
class CThread
{
  ...
  {
    CThread* pstThread = (CThread*)lpVoid;
    pstThread->Run();
    pstThread->OnEnd();
...
if (m_bStart)
    ...
    ResetEvent(m_hQuitEvent);
    ResetEvent(m_hThreadQuitEvent);
    m_hThread = (HANDLE)_beginthreadex(NULL, 0, CThread::ThreadRun, (void*)this, 0, &m_tThread);
    if (m_hThread == INVALID_HANDLE_VALUE)
    {
      return false;
    }
    ...
    int iResult = pthread_create(&m_tThread, &m_tThreadAttr, CThread::ThreadRun, (void*)this);
    if (iResult)
    {
...
if (m_bStart)
  ...
  static int EndSomeOneThread(CThread **ppthread, int ntimeout = -1)
  {
